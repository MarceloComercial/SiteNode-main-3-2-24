const { RegisterSetoresModel } = require('./Connections/connects'); // Importa a biblioteca Mongoose, que facilita a interação com o MongoDB.
...
class RegistrarSetor {
    ...
    async register(){
        ...
        this.user = await RegisterSetoresModel.create(this.body); // Cria um novo registro no banco de dados usando o modelo 'RegisterModel'.
        
    }

    async userExists(){
        const user = await RegisterSetoresModel.findOne({setor: this.body.cadastro_setor}); // Procura por um usuário com o mesmo email no banco de dados.

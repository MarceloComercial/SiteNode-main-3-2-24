...
class Remove {
    ...
    async remove(){
        ...
        this.user = await RegisterModel.deleteOne({ _id: this.body._id }); // Cria um novo registro no banco de dados usando o modelo 'RegisterModel'.
        
    }

    async userExists() {
        
        const user = await RegisterModel.findOne({ _id: this.body._id});
      
        if (!user) this.erros.push('Usuario não existente.');
      }

const { Remove } = require("../model/RemoveGroupModel"); // Importa a classe 'Registrar' do arquivo 'RegistrerModel.js'.
  ...
  try {
    ...
    if (userreg.erros.length > 0) {
      ...
      req.session.save(function () {
        return res.redirect("/editgrupos"); // Redireciona de volta para a página de registro.
      });
      return;
    }
    req.flash("success", 'Grupo Removido com Sucesso.'); // Se o registro for bem-sucedido, armazena uma mensagem de sucesso em um flash message.
    req.session.save(function () {
      return res.redirect("/editgrupos"); // Redireciona de volta para a página de registro.
    });
  } catch (e) {
